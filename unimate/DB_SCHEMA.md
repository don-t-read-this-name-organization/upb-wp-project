...
- Professors can be soft-deleted by setting `active=false` so historical reviews remain.
 ...
 - reviews
  - id: INT, PK, auto-increment
  - user_id: INT, NOT NULL, FK -> `users(id)` (`fk_reviews_user`), ON DELETE RESTRICT
  - professor_id: INT, NOT NULL, FK -> `professors(id)` (`fk_reviews_professor`), ON DELETE RESTRICT
  - rating: INT
...
Rationale:
- Reviews belong to users and professors. Soft-deleting reviews preserves history and avoids FK cascades.
